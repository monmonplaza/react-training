<?php
    try {
        ...
        $genDonation->gen_donation_active  = 1;
        $genDonation->gen_donation_created = date("Y-m-d");  
        $genDonation->gen_donation_datetime = date("Y-m-d H:i:s");  
        
        $result = checkCreate($genDonation);
        Response::sendResponse(true, "Create General Donation success.",  $result);
    }catch (Error $e) {
